...
@InternalApi private[fory] final class ForySerializer(
  ...
  private lazy val fory = ForyInitializer.createThreadLocalForyInstance()
  ...
  private lazy val deserializationType: Option[Class[_ <: AnyRef]] = if (typeInManifest) {
    ...
    configuredDeserializationType.orElse {
      val bindings = PekkoAccessUtil.getSerializationBindings(serialization).filter(_._2.identifier == identifier)
      bindings match {
        case Nil =>
          throw new IllegalArgumentException(
  ...
  override val identifier: Int = PekkoAccessUtil.identifierFromConfig(bindingName, system)
